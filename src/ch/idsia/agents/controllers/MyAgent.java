...
public class MyAgent extends BasicMarioAIAgent implements Agent {
	...
	public boolean[] getAction() {
		/*
		 * System.out.println("x: "+ (int) marioFloatPos[0]/16 + " y: "+ (int)
		 * marioFloatPos[1]/16);
		 * 
		 * i++; System.out.println(i); if (i == 10 && isMarioOnGround ) {
		 * System.out.println((int) marioFloatPos[0]/16); action[Mario.KEY_JUMP]
		 * = true;
		 * 
		 * } else if (i > 10 && isMarioOnGround) {
		 * System.out.println(marioFloatPos[0]/16); action[Mario.KEY_JUMP] =
		 * false; action[Mario.KEY_RIGHT] = false; }
		 * 
		 * if (isMarioAbleToJump || !isMarioOnGround) { action[Mario.KEY_JUMP] =
		 * true; } else{ action[Mario.KEY_JUMP] = false; }
		 */
		if ((isMarioAbleToJump || !isMarioOnGround) && (gapAhead() || obstacleAhead())) {
			 ...
			 * System.out.print(mergedObservation[(mergedObservation.length-1)/2
			 * ][i] + " "); } System.out.println();
			 * System.out.println(mergedObservation[(mergedObservation.length-1)
			 * /2][ (mergedObservation[0].length-1)/2+1]);
			 */
	...
	private boolean gapWithOneStepStairs() {
		boolean b = false;
		if (mergedObservation[mergedObservation.length / 2 - 1][mergedObservation.length / 2 + 2] != 0
				&& mergedObservation[mergedObservation.length / 2 - 1][mergedObservation.length / 2 + 1] == 0) {
			System.out.println(
					"found step at " + (mergedObservation.length / 2 - 1) + " , " + (mergedObservation.length / 2 + 2));
			b = true;
